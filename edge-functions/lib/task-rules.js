const DEFAULT_SUCCESS_RULE = { mode: "any_http" };

const DEFAULT_SCHEDULE_CONFIG = {
  interval: { enabled: true, value: 5, unit: "minutes" },
  fixedTime: { enabled: false, month: null, day: null, hour: 0, minute: 0 },
};

export function normalizeSuccessRule(rule) {
  if (!rule) {
  ...
  if (rule.mode === "custom_codes") {
    ...
    return { mode: "custom_codes", customCodes: [...new Set(codes)] };
  }
...
function legacyScheduleToConfig(schedule) {
  ...
  const amount = Number(String(schedule).replace("m", ""));
...
function normalizeFixedTimeConfig(input) {
...
export function normalizeScheduleConfig(scheduleConfig, legacySchedule) {
  if (!scheduleConfig) {
...
export function getLegacyScheduleFromConfig(config) {
  if (config.interval.enabled) {
    if (config.interval.unit === "hours") {
      return "60m";
    }
    const minutes = config.interval.value;
    return [1, 5, 10, 30, 60].includes(minutes) ? `${minutes}m` : "5m";
  }
  return "single";
}
...
export function shouldTreatStatusCodeAsSuccess(statusCode, successRule) {
  if (statusCode === null || statusCode === undefined) {
    return false;
  }

  const rule = normalizeSuccessRule(successRule);
  if (rule.mode === "any_http") return true;
  if (rule.mode === "2xx_3xx") return statusCode >= 200 && statusCode < 400;
  return (rule.customCodes || []).includes(statusCode);
}

function intervalToMs(config) {
  if (!config.interval.enabled) return 0;
  return config.interval.unit === "hours"
    ? config.interval.value * 60 * 60 * 1000
    : config.interval.value * 60 * 1000;
}

function matchesFixedTime(config, timestamp) {
  if (!config.enabled) return false;
  const parts = getShanghaiParts(timestamp);
  if (config.month !== null && config.month !== parts.month) return false;
  if (config.day !== null && config.day !== parts.day) return false;
  return config.hour === parts.hour && config.minute === parts.minute;
}

function alreadyRanFixedSlot(lastRunTime, timestamp) {
  if (!lastRunTime) return false;
  const prev = getShanghaiParts(lastRunTime);
  const curr = getShanghaiParts(timestamp);
  return prev.month === curr.month && prev.day === curr.day && prev.hour === curr.hour && prev.minute === curr.minute;
}

export function shouldRunTaskNow(task, timestamp) {
...
export function describeScheduleConfig(config, legacySchedule) {
